    ...
    <div className="mt-10 text-center bg-gradient-to-b from-[#213448] to-[#547792] text-white p-8 rounded-xl shadow-lg">
      ...
      >
        <h2 className="mt-4 text-2xl font-bold">
          {userData.name}{" "}
          <span className="font-light">| {userData.login}</span>
        </h2>
      </a>
      ...
      {/* Display total stars and forks count */}
      <div className="mt-4 flex items-center gap-4 justify-center">
        <p>
          <span className="font-semibold">Total Stars:</span>{" "}
          {userData.repos
            ? userData.repos.reduce((total, repo) => total + repo.stargazers_count, 0)
            : 0}
        </p>
        <p>
          <span className="font-semibold">Total Forks:</span>{" "}
          {userData.repos
            ? userData.repos.reduce((total, repo) => total + repo.forks_count, 0)
            : 0}
        </p>
      </div>
    </div>
